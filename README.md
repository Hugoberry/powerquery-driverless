...
The function parses the [SQLite file format](https://www.sqlite.org/fileformat2.html) directly: file header, table b-trees, varints, record serial types, and overflow-page chains. Because it consumes plain bytes, it works anywhere Power Query runs — Power BI Desktop, dataflows, Excel — and it refreshes in the Power BI Service **without a gateway** when the file is reachable via `Web.Contents` or `SharePoint.Files`.

## Why this exists

Power BI has no native SQLite connector. The standard answer is the SQLite ODBC driver, which requires a machine-level (HKLM) installation many corporate users can't perform. The Power BI mashup engine ships no SQLite ADO.NET provider either (its internal SQLite use is native-interop only), and custom connectors cannot bundle binaries. The remaining sanctioned escape hatch — the Python/R script host — requires a configured interpreter and blocks some Service refresh scenarios.

This project removes the dependency entirely by treating the `.db` file as what it is: a well-documented binary format.

## Usage

Paste the contents of `SQLiteReader.pq` into a blank query and name the query `SQLite` (Power Query treats a query whose expression is a function as an invocable function).
...
| **No SQL** | You get whole-table scans; filtering/joining happens in M afterwards. There is no predicate pushdown. |
...
If your `.db` is under constant write load (the scenario that motivated this project):

1. **Copy first, read the copy.** Copy `your.db` (and, if present, `your.db-wal` + `your.db-shm` — though note the WAL contents still won't be parsed) during a quiet window, then point `File.Contents` at the copy.
2. **Checkpoint before refresh** (`PRAGMA wal_checkpoint(TRUNCATE);` from any process that can open the DB) so the main file contains the latest committed state.
...
4. Each leaf cell is `varint payload-length`, `varint rowid`, then the payload — spilled to a chain of overflow pages when it exceeds the usable-size threshold (`X = U − 35`, `M = ((U − 12) × 32 / 255) − 23`, `K = M + (P − M) mod (U − 4)`).
...
- Power BI Desktop, Power BI dataflows, Excel Power Query — anything with a current M engine. No engine extensions or undocumented functions are used.
- Read-only by construction; the source file is never modified.

## Testing your setup

Validate against a known database before trusting production numbers:

```
sqlite3 example.db "select count(*) from orders;"
```

and compare with `Table.RowCount` on the corresponding `Data` table. Good boundary cases to check: negative integers, NULLs in an `INTEGER PRIMARY KEY` column, and text values long enough to span overflow pages.
